...
def main(event_name, data, fs_ref=4096.):
    ...
    for real_ind in realizations_to_eval:
        ...
        for tau in corr_check_range:
            cntr += 1
            cr, _ = p_corr.main(waveform_H, H_extract_, tau)
            corr_arr_H[cntr] = cr
            cr, _ = p_corr.main(waveform_L, L_extract_, tau)
            corr_arr_L[cntr] = cr
    ...
    for tau in corr_check_range:
        cntr += 1
        cr, cr_err = p_corr.main(waveform_H_, H_extract_, tau)
        corr_arr_H[cntr] = cr
        cr, cr_err = p_corr.main(waveform_L_, L_extract_, tau)
        corr_arr_L[cntr] = cr
